...
export class BossAI {
  ...
  private updateWarning(dt: number, bossPos: THREE.Vector3, playerPos: THREE.Vector3): void {
    ...
    if (this.activeAttack.warningMesh) {
      ...
      mat.opacity = 0.35 + Math.sin(Date.now() * 0.012) * 0.15;
    }
  ...
  private resolveAttack(bossPos: THREE.Vector3, playerPos: THREE.Vector3): void {
    ...
    const hit = this.checkHit(this.activeAttack, bossPos, playerPos);
    ...
    if (hit) {
      this.state = 'ATTACK';
      this.stateTimer = 0.15; // brief hit frame
    } else {
      // Player not in area → miss, BOSS recovery
      this.activeAttack = null;
      this.state = 'RECOVERY';
      this.stateTimer = 1.5;
    }
  ...
  /** Clean up warning meshes from scene. */
